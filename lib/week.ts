...
// Newest first (yesterday, then the day before, ...) - today's dinner stays
// on This Week until the day is actually over.
export function getPastDays(count: number): UpcomingDay[] {
  const now = new Date();
  const days: UpcomingDay[] = [];
  for (let i = 1; i <= count; i++) {
    ...
    const weekday = d.toLocaleDateString("en-US", { weekday: "short" });
    const label = `${weekday} ${d.getMonth() + 1}/${d.getDate()}`;
    const relative = i === 1 ? "Yesterday" : null;
    days.push({ date, label, relative });
  }
  return days;
}
