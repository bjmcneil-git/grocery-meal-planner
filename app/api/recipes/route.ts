...
export async function POST(req: NextRequest) {
  ...
  const { name, instructions, source, source_url, cuisine, image_url, cook_time_minutes, ingredients } = body;
  ...
  const [recipe] = await d1Query<Recipe>(
    `INSERT INTO recipes (id, name, source, source_url, instructions, cuisine, image_url, cook_time_minutes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)
     RETURNING *`,
    [
      id,
